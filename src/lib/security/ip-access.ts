 ...
 * Handles standard proxies, Vercel, Cloudflare, AWS ALB, and direct connections.
 */
...
export function isIpAllowed(clientIp: string, allowedEntry: string): boolean {
  ...
  // Exact match (covers IPv4 and IPv6 like 127.0.0.1 or ::1)
  if (clientIp === entry) return true;

  // Localhost aliases
  if ((clientIp === "::1" || clientIp === "127.0.0.1") && (entry === "127.0.0.1" || entry === "::1" || entry === "localhost")) {
    return true;
  }

  // CIDR match (e.g. 192.168.1.0/24 or 10.0.0.0/8)
  if (entry.includes("/")) {
...
<body>
  <div class="card">
    ...
    <p>This domain is protected by strict IP whitelisting. Your current IP address is not authorized to access this resource.</p>
    <div class="meta">
      <div><span class="label">Domain:</span> ${escapeHtml(domain)}</div>
      <div><span class="label">Client IP:</span> ${escapeHtml(clientIp)}</div>
    </div>
  </div>
</body>
</html>`;
