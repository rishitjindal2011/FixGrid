...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.error) {
      ...
      setNotice("Inventory session authorized. Redirecting to surplus reclamation portal...");
      setTimeout(() => {
  ...
  return (
    <AuthShell
      ...
      intro="Access your workshop inventory terminal to list bench-tested replacement hardware and overstock components."
      footer={
        <>
          Need a registered workshop account?{" "}
    ...
    >
      <form onSubmit={handleSubmit} className="flex flex-col gap-4.5" noValidate>
        ...
        <Field label="Workshop Password" htmlFor="password">
          <input
            id="password"
            type="password"
            required
        ...
        >
          {loading ? (
            <>
              ...
              <span>Authorizing Inventory Session...</span>
            </>
          ) : (
            <>
              <span>Sign In to Parts Portal</span>
        ...
        <div className="pt-2 text-center">
          <span className="font-mono text-[11px] text-steel-soft flex items-center justify-center gap-1.5">
            ...
            FixGrid Encrypted Supabase Auth &middot; parts.vytron.me
          </span>
        </div>
      </form>
    </AuthShell>
