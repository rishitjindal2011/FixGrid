...
/** Upload customer dissatisfaction / revision evidence photos. */
export async function uploadBookingEvidencePhotos(
  bookingId: string,
  files: File[],
): Promise<FaultUploadResult> {
  if (files.length === 0) return { uploaded: 0, failed: 0 };
  ...
  for (const file of files.slice(0, MAX_FILES)) {
    ...
    const objectPath = `${bookingId}/evidence/${randomId()}-${safeName(file.name) || "photo"}`;
    ...
    if (uploadError) {
      console.error("[bookings] evidence photo upload failed", uploadError.message);
      failed += 1;
      continue;
    }
    ...
    if (rowError) {
      console.error("[bookings] evidence photo row failed", rowError.message);
      failed += 1;
      continue;
    }
