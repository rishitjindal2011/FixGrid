 ...
 * feature: this function is the only thing turning anonymous requests away from
 * the dashboard, so a silent no-op publishes the entire admin.
 *
...
export async function proxy(request: NextRequest) {
  ...
  if (!session && !isLoginRoute) {
    ...
    // page validates it before redirecting — an absolute URL here would be an
    // open redirect.
...
export const config = {
  /*
   * Everything except Next's own assets. The API routes are *included*
   * deliberately: /api/preview and /api/export-html both act on behalf of a
   * signed-in admin and must not be reachable anonymously.
   */
