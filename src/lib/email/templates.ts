...
export function renderEmailTemplate(input: EmailTemplateInput): string {
  ...
  const unsubscribeUrl =
    ...
    (input.recipientEmail
      ? `${siteUrl}/unsubscribe?email=${encodeURIComponent(input.recipientEmail)}`
      : `${siteUrl}/unsubscribe`);
...
export function renderPlainText(input: EmailTemplateInput): string {
  ...
  lines.push(
    ...
    input.unsubscribeUrl ??
      (input.recipientEmail
        ? `${CANONICAL_ORIGIN}/unsubscribe?email=${encodeURIComponent(input.recipientEmail)}`
        : `${CANONICAL_ORIGIN}/unsubscribe`),
    "",
