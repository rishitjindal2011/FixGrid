...
import { X, ShieldCheck, Wrench, Lock, ArrowRight, CheckCircle2, AlertCircle, Loader2 } from "lucide-react";
...
export function AuthModal({ isOpen, onClose, defaultMode = "signin", onSuccess }: AuthModalProps) {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (res.error) {
      ...
      setSuccessMsg("Bench credentials verified. Access granted.");
      setTimeout(() => {
        setSuccessMsg(null);
        onClose();
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (res.error) {
      ...
      setSuccessMsg("Workshop account created! Welcome to FixGrid Bench.");
      setTimeout(() => {
        setSuccessMsg(null);
        onClose();
        if (onSuccess) onSuccess();
  ...
  const handleRegisterWorkshop = async (e: React.FormEvent) => {
    ...
    if (res.error) {
      ...
      setSuccessMsg("Workshop registered and authenticated!");
      setTimeout(() => {
        setSuccessMsg(null);
        onClose();
        if (onSuccess) onSuccess();
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl border border-zinc-700/80 bg-zinc-900 shadow-2xl z-10 font-sans text-zinc-100 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <h2 className="text-base font-bold text-white tracking-tight">
                  ...
                  {mode === "register_workshop" && "Register Bench Profile"}
                </h2>
              </div>
            </div>
        ...
        <div className="p-6">
          ...
          {user && !workshop && (
            <div className="mb-4 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3.5 text-xs">
              ...
              <p className="text-zinc-300 mt-1">
                To post bench seats and job openings, register your workshop bench details below:
              </p>
            </div>
          )}
            ...
            <form onSubmit={handleSignIn} className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block font-mono text-[11px] font-medium text-zinc-400 uppercase">
                    Bench Password
                  </label>
                </div>
                <input
              ...
              >
                {loading ? (
                  <>
                    ...
                    <span>Verifying Bench ID...</span>
                  </>
                ) : (
                  <>
                    <span>Sign In & Access Bench</span>
          ...
          <div className="mt-5 border-t border-zinc-800/80 pt-4 flex items-center justify-between text-[11px] text-zinc-400">
            ...
            <span className="font-mono text-zinc-400">Domain: hiring.vytron.me</span>
          </div>
        </div>
      </div>
    </div>
