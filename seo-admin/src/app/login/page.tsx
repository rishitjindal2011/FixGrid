...
/**
 * The proxy already sends signed-in visitors away from this route, so this
 * page does not re-check the session — it would be a second verify on every
 * render for a case that cannot reach here.
 *
 * `next` arrives as a search param from the proxy redirect and is passed
 * through to a hidden field. It is validated again inside the login action:
 * a hidden field is user input, and the value has made a round trip through the
    ...
    <main className="grid min-h-dvh place-items-center px-4 py-12">
      <div className="w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center gap-3 text-center">
          ...
          <div>
            <h1 className="text-display-sm">SEO Admin</h1>
            <p className="mt-1 text-sm text-steel">FixGrid content console</p>
          </div>
        </div>
