...
async function uniqueSlug(admin: ReturnType<typeof createAdminClient>, name: string): Promise<string> {
  const base = slugify(name) || "stockist";
  ...
  try {
    ...
    if (!shopName || !address || !contactPhone || !contactEmail) {
      return NextResponse.json(
        { error: "Workshop/Stockist name, address, contact phone, and official email are required." },
        { status: 400 }
      );
    }
    ...
    if (!userId) {
      ...
      if (existingUser) {
        ...
        const { data: newUser, error: createError } = await admin.auth.admin.createUser({
          email: contactEmail,
          password: password || "VytronParts2026!",
          email_confirm: true,
          user_metadata: {
            full_name: shopName,
            role: "shop",
        ...
        if (createError) {
          console.warn("[join-submit-parts] User creation notice:", createError.message);
      ...
      for (const item of evidenceBase64) {
        try {
          ...
          if (!uploadError) {
            ...
            console.warn("[join-submit-parts] Storage upload notice:", uploadError.message);
            finalEvidencePaths.push(`[File: ${item.name || "Evidence file"}]`);
          }
        } catch (storageErr) {
          console.warn("[join-submit-parts] Error processing file attachment:", storageErr);
        }
      }
    }
    ...
    const bioText = [
      specialization ? `Parts Category: ${specialization}` : null,
      legalName ? `Legal Entity: ${legalName}` : null,
      taxId ? `GSTIN / Tax ID: ${taxId}` : null,
      benchCapacity ? `Warehouse / Bench Capacity: ${benchCapacity} stations` : null,
    ]
      .filter(Boolean)
      .join(" | ");
    ...
    if (shopError) {
      console.error("[join-submit-parts] Failed to insert fixer profile:", shopError);
      return NextResponse.json(
        { error: `Database error creating stockist profile: ${shopError.message}` },
        { status: 500 }
      );
    }
    ...
    const evidenceTextLines = [
      "[Portal: parts.vytron.me]",
      legalName ? `Legal Registered Entity: ${legalName}` : null,
      taxId ? `GSTIN / Business Registration: ${taxId}` : null,
      specialization ? `Component Inventory Specialization: ${specialization}` : null,
      benchCapacity ? `Warehouse Stations: ${benchCapacity}` : null,
      contactEmail ? `Official Contact Email: ${contactEmail}` : null,
      notes ? `Inventory & Dispatch Notes: ${notes}` : null,
      finalEvidencePaths.length > 0
        ? `Uploaded evidence (${finalEvidencePaths.length}):\n${finalEvidencePaths.map((p) => `• ${p}`).join("\n")}`
        : "Evidence: Physical warehouse and multimeter testing jig verification requested via parts onboarding desk.",
    ].filter(Boolean);
    ...
    if (claimError) {
      console.error("[join-submit-parts] Failed to create shop claim:", claimError);
    ...
    return NextResponse.json({
      ...
      is_hidden: true,
      portal: "parts",
      message: "Parts supplier registration submitted for administrative review.",
    });
  } catch (err: any) {
    console.error("[join-submit-parts] Unexpected error:", err);
    return NextResponse.json(
      { error: err.message || "Failed to process parts supplier registration." },
      { status: 500 }
    );
  }
}
