...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-machined font-display uppercase tracking-wide transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        // Signal orange is the only "do this" colour in the system.
        ...
        outline:
          "border border-hairline bg-chalk text-enamel hover:bg-bench hover:border-steel-soft",
...
export interface ButtonProps
  ...
  /** Render the child element instead of a `<button>` — for `<Link>` actions. */
  asChild?: boolean;
}
